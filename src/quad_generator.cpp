...
{
    auto ret =
        new (quadAddrPool.Alloc()) QuadAddr(type, master, minion, islval);
    ret->m_pool = &quadAddrPool;
    return ret;
}
...
void QuadGenerator::VisitBinaryOp(BinaryOp *binary)
{
    ...
    if (binary->IsConst() && binary->m_lhs->IsConst() &&
        ...
        EmitComment("combining %d %s %d to %d", binary->m_lhs->m_val,
                    tokenlist[binary->m_op], binary->m_rhs->m_val,
                    binary->m_val);
        return;
    }
    ...
    for (auto slot : m_curfunc->m_quads) {
        if (slot->m_op == QO_PARAM) {
            if (slot->m_dst->m_resolved == false) {
                slot->m_dst->m_offset =
                    m_curfunc->m_frame.size + slot->m_dst->m_offset;
        ...
        if (slot->m_dst) {
            if (slot->m_dst->m_type == QuadAddr::AT_ARRAY) {
                ...
                mapoffset(slot->m_dst->m_minion, m_curfunc->m_frame.size,
                          save_size);
            }
            if (slot->m_dst->m_resolved == false) {
                slot->m_dst->m_offset =
        ...
        if (slot->m_arg1) {
            if (slot->m_arg1->m_type == QuadAddr::AT_ARRAY) {
                mapoffset(slot->m_arg1->m_minion, m_curfunc->m_frame.size,
                          save_size);
            }
            if (slot->m_arg1->m_resolved == false) {
                slot->m_arg1->m_offset = m_curfunc->m_frame.size - save_size -
        ...
        if (slot->m_arg2) {
            if (slot->m_arg2->m_resolved == false) {
                slot->m_arg2->m_offset = m_curfunc->m_frame.size - save_size -
...
void QuadGenerator::VisitVarDecl(VarDecl *varDecl)
{
    ...
    if (m_curfunc) {
        ...
        auto qa = QuadAddr::New(QuadAddr::AT_IDENT, varDecl->m_name, false,
                                m_curoffset);
        ...
        if (varDecl->IsConstQualified()) {
            auto type =
                varDecl->IsChar() ? QuadAddr::AT_CHARL : QuadAddr::AT_INTL;
...
void QuadGenerator::VisitParmVarDecl(ParmVarDecl *parmVarDecl)
{
    ...
    m_curqa = QuadAddr::New(QuadAddr::AT_IDENT, parmVarDecl->m_name, false,
                            m_curoffset);
...
std::string Quadruple::Str()
{
    ...
    case QO_ASSIGN:
        ...
        break;
    case QO_PLUS:
        ret += "plus " + m_dst->Str() + " = " + m_arg1->Str() + " + " +
               m_arg2->Str();
        break;
    case QO_MINU:
        ret += "minu " + m_dst->Str() + " = " + m_arg1->Str() + " - " +
               m_arg2->Str();
        break;
    case QO_MULT:
        ret += "mult " + m_dst->Str() + " = " + m_arg1->Str() + " * " +
               m_arg2->Str();
        break;
    case QO_DIV:
        ret += "div " + m_dst->Str() + " = " + m_arg1->Str() + " / " +
               m_arg2->Str();
        break;
    case QO_INDEX_ARG:
    ...
    case QO_BLT:
        ret =
            "blt " + m_dst->Str() + ", " + m_arg1->Str() + ", " + m_arg2->Str();
        break;
    case QO_BLE:
        ret =
            "ble " + m_dst->Str() + ", " + m_arg1->Str() + ", " + m_arg2->Str();
        break;
    case QO_BGT:
        ret =
            "bgt " + m_dst->Str() + ", " + m_arg1->Str() + ", " + m_arg2->Str();
        break;
    case QO_BGE:
        ret =
            "bge " + m_dst->Str() + ", " + m_arg1->Str() + ", " + m_arg2->Str();
        break;
    case QO_BEQ:
        ret =
            "beq " + m_dst->Str() + ", " + m_arg1->Str() + ", " + m_arg2->Str();
        break;
    case QO_BNE:
        ret =
            "bne " + m_dst->Str() + ", " + m_arg1->Str() + ", " + m_arg2->Str();
        break;
    default:
        break;
    }
